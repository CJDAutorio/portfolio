...
import { aboutMeContent, EducationExperience, ProjectExperience, WorkExperience } from './types';
...
export async function getAllExperienceDocs() {
    const querySnapshot = await getDocs(collection(FirestoreDB, "experience-content"));
    const content: (WorkExperience | ProjectExperience | EducationExperience)[] = [];

    const promises = querySnapshot.docs.map(async (doc) => {
        const experienceContent = doc.data() as WorkExperience | ProjectExperience | EducationExperience;
        console.log(`${doc.id}`, doc.data());
        experienceContent.id = doc.id;
        if (experienceContent.media) {
            const url = await firebaseStorageGetDownloadURL(experienceContent.media as string);
            console.log('url:', url);
